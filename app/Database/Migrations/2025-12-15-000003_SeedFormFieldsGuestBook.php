...
class SeedFormFieldsGuestBook extends Migration
{
    public function up()
    {
        // Check if guest_book form fields already exist
        $existingCount = $this->db->table('form_fields')
            ->where('form_type', 'guest_book')
            ->countAllResults();

        if ($existingCount == 0) {
            // Insert guest book form fields
            $guestBookFields = [
                [
